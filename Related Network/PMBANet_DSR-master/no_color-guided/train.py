...
parser.add_argument('--hr_train_dataset', type=str, default='train_hr_all_256/')
#parser.add_argument('--train_dataset', type=str, default='DIV2K_LR_aug_x8')
parser.add_argument('--train_dataset', type=str, default='train_x4_all_256/')
...
parser.add_argument('--pretrained_sr', default="PMBA_x4.pth", help='sr pretrained base model')
parser.add_argument('--pretrained', type=bool, default=True)
parser.add_argument('--save_folder', default='./weights/half_size_one_channel/', help='Location to save checkpoint models')
parser.add_argument('--prefix', default='PMBAX4', help='Location to save checkpoint models')
...
if opt.model_type == 'PMBAX4':
    model = PMBAX4(num_channels=1, base_filter=64,  feat=256, num_stages=3, scale_factor=opt.upscale_factor)
else:
    model = PMBAX4(num_channels=1, base_filter=64,  feat=256, num_stages=5, scale_factor=opt.upscale_factor)
#############x############################################################################################
...
for epoch in range(1, opt.nEpochs + 1):
    ...
    if (epoch+2) % 10 == 0:
        ...
        plt.savefig('log_loss_PMBAX4.png')
